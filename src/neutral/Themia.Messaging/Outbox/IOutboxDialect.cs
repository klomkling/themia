...
/// path. One implementation per database engine, per outbox table.
/// </summary>
/// <typeparam name="TRow">The claimed-row shape this outbox stores.</typeparam>
public interface IOutboxDialect<TRow>
    where TRow : IClaimedRow
{
    ...
    /// Rows scheduled for the future are not claimed.
    /// </summary>
    ...
    Task<IReadOnlyList<TRow>> ClaimAsync(
    ...
    /// <summary>Marks a claimed row as delivered and clears its lease.</summary>
    /// <param name="connection">An open connection from <see cref="CreateConnection"/>.</param>
    /// <param name="id">The row to complete.</param>
    /// <param name="completedAt">The delivery timestamp recorded on the row.</param>
    /// <param name="ct">A token to cancel the operation.</param>
    /// <returns>A task that completes when the row has been updated.</returns>
    Task CompleteAsync(DbConnection connection, Guid id, DateTimeOffset completedAt, CancellationToken ct);
