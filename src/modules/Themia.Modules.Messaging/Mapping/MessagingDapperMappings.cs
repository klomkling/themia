...
/// <summary>Registers the Themia Messaging entity mappings (the prefixed <c>messaging_outbox_messages</c>
/// table name) into a Dapper <see cref="EntityMappingRegistry"/>, so the Dapper peer reads and writes the
/// exact same columns as the EF peer over the FluentMigrator-owned schema.</summary>
public static class MessagingDapperMappings
{
    ...
    public static void Apply(EntityMappingRegistry registry)
    {
        ...
        registry.Register<MessageOutboxEntry>(
            EntityMapping.ForConvention<MessageOutboxEntry>("messaging_outbox_messages", null));
    }
}
