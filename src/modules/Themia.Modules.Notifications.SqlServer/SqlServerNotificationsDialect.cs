...
/// (Microsoft.Data.SqlClient). The claim is a single atomic statement: <c>UPDATE TOP (@batch) ... WITH
/// (READPAST, UPDLOCK, ROWLOCK) ... OUTPUT inserted.*</c>, so concurrent drainers skip each other's
/// locked rows and no explicit transaction is needed.</summary>
