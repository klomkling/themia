...
/// <summary>
/// Proves <c>AddThemiaExceptionalSqlServer</c> migrates a fresh database on its own, through
/// <c>SqlServerMigrationEngine.Adapter</c> directly rather than through <c>MigrationEngineRegistry</c>
/// (design spec §5.3, Task 3). <see cref="SqlServerExceptionStoreTests"/> builds <c>ExceptionStoreEngine</c>
/// by hand and migrates via <c>ThemiaMigrations.Run(MigrationEngine.SqlServer, …)</c> directly — see
...
public sealed class SqlServerExceptionalEntryPointTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task AddThemiaExceptionalSqlServer_migrates_a_fresh_database()
    {
        var services = new ServiceCollection();
        services.AddThemiaExceptionalSqlServer(container.GetConnectionString(), o => o.ApplicationName = "App");
