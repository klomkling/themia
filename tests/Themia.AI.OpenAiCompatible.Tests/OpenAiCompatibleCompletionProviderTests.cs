...
public sealed class OpenAiCompatibleCompletionProviderTests
{
    ...
    // ---- Transport failure ----
    //
    // The headline deployment for this provider is a local Ollama or LM Studio at BaseUrl. A stopped one
    // refuses every connection, and every other handler in this suite answers with a status code, so
    // nothing here could reach that path: HttpClient throws HttpRequestException and no response exists.
    // Before this test the exception escaped CompleteAsync, so the dispatcher never saw ProviderError —
    // and a stopped local server, the exact case the cloud provider behind it in Failover was configured
    // for, ended the whole call instead of failing over.
    [Fact]
    ...
    // whose body is not the JSON this parses — a proxy's error page, or a body cut short — threw
